...
Purpose:
    Official GPU batch-size-1 inference benchmark for the strengthened
    ...
    single-image GPU inference latency with batch_size=1.
...
Typical commands:
    python main_scripts\benchmark_gpu_batch1_3datasets.py --dataset eurosat --models mobilenet_v2 --smoke-test
    python main_scripts\benchmark_gpu_batch1_3datasets.py --dataset eurosat --models all
    python main_scripts\benchmark_gpu_batch1_3datasets.py --dataset aid --models all
    python main_scripts\benchmark_gpu_batch1_3datasets.py --dataset resisc45 --models all
...
DEFAULT_RESULTS_PATH = PROJECT_ROOT / "results" / "gpu_benchmark_results_batch1_3datasets.csv"
...
def get_device() -> torch.device:
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA is not available. Do not use CPU fallback for official GPU benchmark results."
        )
...
def run_one_model(
    ...
    print_banner(f"GPU BATCH-1 BENCHMARK | {display_name} | {model_name}")
    ...
    print("[INFO] Measuring official GPU single-image latency at batch_size=1 ...")
    ...
    print(f"Peak GPU VRAM:  {latency.gpu_peak_vram_mb:.6f} MB")
    ...
    row = {
        ...
        "gpu_peak_vram_mb": f"{latency.gpu_peak_vram_mb:.6f}",
        ...
        "history_csv": "",
        "notes": (
            "Official GPU batch-size-1 single-image latency rerun; no retraining; "
            "PyTorch CUDA FP32 checkpoint; run under AC power and Turbo/performance mode"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Official GPU batch-size-1 inference benchmark for EuroSAT/AID/RESISC45."
    )
    parser.add_argument(
        "--dataset",
        type=str,
...
def main() -> None:
    ...
    print_banner("OFFICIAL GPU BATCH-1 SINGLE-IMAGE LATENCY BENCHMARK")
    ...
    print_banner("DONE")
    print(f"[OK] GPU batch-1 benchmark finished: {args.results_path}")
